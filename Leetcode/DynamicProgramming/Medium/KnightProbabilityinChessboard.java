...
public class KnightProbabilityinChessboard {
	...
	/*
	 * Naively the dp is a 3-dimensional array. But we only need the previous
	 * one to derive the current one, so I only preserve the previous one in the
	 * dp0 and calculate the current one in the dp1.
	 * 
	 * Let’s think about our formula, dp1[i][j] = sum(dp0[all reachable spots]).
	 * Assuming we are calculating the first step and all reachable spots are
	 * within the chessboard. Then the dp should be 8, which implies the
	 * initializing value should be 1.
	 */
	// better approach with bottom converting dp from 3d in top down to 2d in
	// bottom up
	class BottomUp {
