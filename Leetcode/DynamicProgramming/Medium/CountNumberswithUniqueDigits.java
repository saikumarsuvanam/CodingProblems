...
public class CountNumberswithUniqueDigits {
	 ...
	 * 
	 * The problem is asking for numbers from 0 to 10^n. Hence return f(1) +
	 * f(2) + … + f(n)
	 * 
	 * As @4acreg suggests, There are only 11 different ans. You can create a
	...
	public int countNumbersWithUniqueDigits(int n) {
		...
		int prev = 81;
		int res = 91;
		int availabledigits = 8;
		for (int i = 3; i <= n; i++) {
			prev = prev * (availabledigits);
			res = res + prev;
			availabledigits--;
		}
		return res;
