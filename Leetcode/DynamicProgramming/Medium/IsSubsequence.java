...
//	PreProcess:
//	First Store each character in t in a map wth key as map and values as indices of key appearing
//	at different positions in t
//	Query:
//		for every character in String s, get the values from the key and do the binary search to get 
//		the value
	
	// Follow-up: O(N) time for pre-processing, O(MlogN) for each S.where n is the length of T  and M is the length of S
...
//	
	...
	class MultipleQueries {
		...
		int BinarySearch(int low, int high, int prev, List<Integer> li) {
			while (low < high) {
				int mid = (low + high) / 2;
				if (prev < (int) li.get(mid)) {
					high = mid;
				} else if (prev >= (int) li.get(mid)) {
					low = mid + 1;
				}
			}
			if (li.get(low) > prev) {
